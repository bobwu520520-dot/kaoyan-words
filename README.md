...
| 页面 | 说明 |
|---|---|
| `study.html` | 背单词：分层（核心高频 → 高频重点 → 重点扩展 → 普通扩展），间隔 1/3/7/15/30/60 天复习，进度保存在浏览器 localStorage，可导出/导入；页内可直接配置 AI |
...
| 高频重点 | 1308 | 阅读、完形、翻译重点（含 108 个补录基础高频词） |
| 重点扩展 | 1062 | 学术、社会、科技等主题词，全部含本地释义 |
| 普通扩展 | 633 | 低频与补全词，全部含本地释义，后期查漏补缺 |

**4003 词全部含本地中文释义**。每个词条包含：音标、词性、中文释义、**考研核心义（exam_meaning）**、**熟词僻义（secondary_meanings）**、例句、**高频搭配（collocation_hint）**、**词族/词形/同反义词/易混词（精选词）**、**数据完整度等级（quality_score）**等字段（完整字段规范见 DATA_QUALITY.md）。考研核心义与熟词僻义均为人工精选、词典可查实的义项；无法可靠判断的词不填，绝不伪造。查词详情页显示数据完整度徽章（如 `████████░░ 80% · B级`，与考频无关）。
...
├── data/
│   ├── words.json      # 4003 词主词库（页面运行时唯一使用的数据）
